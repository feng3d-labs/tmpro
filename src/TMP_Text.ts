import { Component, oav, Material, Color4, Vector4 } from 'feng3d';
import { FontStyles } from './enums/FontStyles';
import { TextAlignmentOptions } from './enums/TextAlignmentOptions';
import { TextOverflowModes } from './enums/TextOverflowModes';
import { TextureMappingOptions } from './enums/TextureMappingOptions';
import { VertexSortingOrder } from './enums/VertexSortingOrder';
import { TMP_ColorGradient } from './TMP_ColorGradient';
import { TMP_FontAsset } from './TMP_FontAsset';
import { TMP_SpriteAsset } from './TMP_SpriteAsset';
import { VertexGradient } from './utils/VertexGradient';

export abstract class TMP_Text extends Component
{
    /**
     * A string containing the text to be displayed.
     */
    @oav({ tooltip: 'A string containing the text to be displayed.' })
    text = 'New Text';
     ...
     */
    @oav({ block: 'Main Settings', tooltip: 'The Font Asset to be assigned to this text object.' })
    font: TMP_FontAsset;

    /**
     * The material to be assigned to this text object. An instance of the material will be assigned to the object's renderer.
     */
    @oav({ block: 'Main Settings', tooltip: 'The material to be assigned to this text object. An instance of the material will be assigned to the object\'s renderer.' })
    fontMaterial = Material.getDefault('TextMeshPro-Material');

    /**
     * Styles to apply to the text such as Bold or Italic.
     */
    @oav({ block: 'Main Settings', tooltip: 'Styles to apply to the text such as Bold or Italic.', component: 'OAVEnum', componentParam: { enumClass: FontStyles } })
    fontStyle = FontStyles.Normal;

    /**
     * The size the text will be rendered at in points.
     */
    @oav({ block: 'Main Settings', tooltip: 'The size the text will be rendered at in points.' })
    fontSize = 36;
     ...
     */
    @oav({ block: 'Main Settings', tooltip: 'Auto sizes the text to fit the available space.' })
     ...
     * The gradient color applied over the Vertex Color. Can be locally set or driven by a Gradient Asset.
     */
    colorGradient = new VertexGradient(new Color4(1, 1, 1, 1));
